    ...
    def _clear_simple_buffer(self):
        ...
        for index, w in enumerate(self.simple_buffer):
            ...
            if w[0] in ["log_format"] and num > 3:
                ...
                self.export_content += (
                    self._indent() + w[0].ljust(max_w) + SPACE_W * " " + w[1]
                )
                ...
                if index > 0 and self.simple_buffer[index - 1][0] == w[0]:
                    justify = True
                if (
                    index + 1 < len(self.simple_buffer)
                    and self.simple_buffer[index + 1][0] == w[0]
                ):
                    justify = True
                ...
                if justify:
                    ...
                    for i in xrange(1, num):
                        self.export_content += SPACE_W * " " + w[i].ljust(
                            items_max_w[w[0]][i]
                        )
                else:
                    self.export_content += self._indent() + w[0].ljust(max_w)
...
def main():
    ...
    parser.add_argument(
        "-f", action="store", metavar="nginx.conf", dest="conf_file", help="conf file"
    )
    parser.add_argument(
        "-d", action="store", metavar="conf.d", dest="conf_dir", help="conf directory"
    )
